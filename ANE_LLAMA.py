...
class LlamaSdpaAttention(modeling_llama.LlamaSdpaAttention):
	...
	def forward(
		...
		#after hidden_states, shape: torch.Size([1, 2048, 10, 1])
		print(f"after hidden_states, shape:", hidden_states_reshaped.shape)

		query_states = self.q_proj(hidden_states_reshaped) # Is this being applied to the conv2d that I ref above? or is it being applied to the linear layer that is from modeling_llama.LlamaAttention
		print(f"query_states shape llamasdpaattention: ", query_states.shape) #  torch.Size([1, 10, 1, 2048]) -> hidden_states_reshaped : torch.Size([1, 2048, 10, 1])

		# I see what the issue is.... hiden_states (linear), self.k_proj is conv2d, cannot do anything, need conv2d format
		key_states = self.k_proj(hidden_states_reshaped)#.unsqueeze(2) #torch.Size([1, 10, 1, 256]) ->torch.Size([1, 256, 10, 1])
		print(f"key_states shape llamasdpaattention: ", key_states.shape)

		value_states = self.v_proj(hidden_states_reshaped)#.unsqueeze(2) #torch.Size([1, 10, 1, 256]) -> torch.Size([1, 256, 10, 1])
		...
		print(f"query_states shape view: ", query_states.shape) # torch.Size([1, 10, 32, 64])

		key_states = key_states.view(bsz, self.num_key_value_heads, q_len, self.head_dim).permute(0, 2, 1, 3)
		print(f"key_states shape view: ", key_states.shape) # torch.Size([1, 10, 4, 64])

		value_states = value_states.view(bsz, self.num_key_value_heads, q_len, self.head_dim).permute(0, 2, 1, 3)
		print(f"value_states shape view: ", value_states.shape) #torch.Size([1, 10, 4, 64])
		
		## so conversion -> computation -> conversion -> let data flow through the rest of the program
		...
		#*** IS THIS NEEDED YES OR NO? ***#
		#key_states = repeat_kv(key_states, self.num_key_value_groups)
		print(f"key_states needs to be (batch, num_attention_heads, seqlen, head_dim)", key_states.shape) # after repeat_kv, torch.Size([1, 80, 4, 64]), i dont have 80 num_attention_heads
		#value_states = repeat_kv(value_states, self.num_key_value_groups)
		print(f"value_states needs to be (batch, num_attention_heads, seqlen, head_dim)", value_states.shape)  # after repeat_kv, torch.Size([1, 80, 4, 64]), i dont have 80 num_attention_heads
		...
		print(f"query_states shape: ", query_states.shape) #torch.Size([1, 10, 32, 64])
		print(f"value_states shape: ", value_states.shape) #torch.Size([1, 10, 4, 64]) - should it be this??? 
		print(f"key_states shape: ", key_states.shape) #torch.Size([1, 10, 4, 64])	- should it be this??? 
		...
		attn_output = (query_states @ key_states.transpose(-2, -1)) * (1.0 / math.sqrt(key_states.size(-1)))
		print(f"attn_output shape", attn_output.shape)
		#reduced_attention_mask = attention_mask[:, :, :10, :4].squeeze()
		#compatible_attention_mask = reduced_attention_mask.mean(dim=2, keepdim=True).expand_as(attn_output)
		attn_output = attn_output #+ compatible_attention_mask
		...
		#attn_output = batch size, seq length, hidden size, number of attn heads
		return attn_output, None, past_key_value  
...
class LlamaModel(modeling_llama.LlamaModel):
	...
	def __init__(self, config):
		...
		self._register_load_state_dict_pre_hook(linear_to_conv2d_map)
		setattr(self, 'layers', nn.ModuleList([LlamaDecoderLayer(config, layer_idx=0) for _ in range(config.num_hidden_layers)])) 
	...
	def forward(
		...
		for decoder_layer in self.layers:
			...
			if self.gradient_checkpointing and self.training:
				#raise NotImplementedError("not implemented gradient checkpointing and self training")
...
"""
class LlamaForSequenceClassification(modeling_llama.LlamaForSequenceClassification):
...
"""
class LlamaForQuestionAnswering(modeling_llama.LlamaForQuestionAnswering):
	def __init__(self, config):
		super().__init__(config)
		#setattr(self,)
